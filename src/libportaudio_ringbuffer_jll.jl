module libportaudio_ringbuffer_jll
...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libportaudio_ringbuffer_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("8c9f9fde-75ea-5170-83a2-66d24179d9fc"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libportaudio_ringbuffer", artifacts_toml) for e in artifacts["libportaudio_ringbuffer"]]
...
if best_platform === nothing
    @debug("Unable to load libportaudio_ringbuffer; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module libportaudio_ringbuffer_jll
